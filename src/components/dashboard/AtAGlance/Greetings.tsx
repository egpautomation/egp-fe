...
const Greetings = () => {
  ...
  const inActiveMigrations = companyMigrations?.filter(
        ...
        <div className="flex md:gap-8 md:mt-8">
          <div className="p-3 md:p-5 ">
            <div className="flex items-center gap-2">
              ...
              >
                <RefreshCw className={`w-4 h-4 text-gray-500 hover:text-gray-700 ${isRefreshing ? 'animate-spin' : ''}`} />
              </Button>
            </div>
      ...
      <div className="">
        <div className="flex flex-wrap justify-between py-1.5  px-3 md:p-5 border-b-2 bg-red-200 text-orange-800 font-semibold">
          <p className=" text-sm md:text-base ">
            <Dot className="inline-block" /> {inActiveMigrations?.length} Egp Id
            did not ready to order a job
          </p>
          <Link to={"/dashboard/my-registered-company"} className="text-sm md:text-base w-max">
            <span className=" flex">
              Update Egp Id Information{" "}
        ...
        <div className="flex flex-wrap justify-between py-1.5  px-3 md:p-5   text-gray-600 font-semibold">
          <p className=" text-sm md:text-base ">
            <Dot className="inline-block text-blue-700" /> {tendersCount}{" "}
            Tenders are live — send us your job orders to proceed.
          </p>
          <Link
            to={"/dashboard/live-tenders"}
            className="text-sm md:text-base w-max text-blue-700"
          >
            <span className=" flex">
