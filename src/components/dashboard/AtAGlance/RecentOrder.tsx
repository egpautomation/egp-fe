        ...
        <div className="flex max-md:flex-col items-center gap-3">
          ...
          >
            <button className="border w-full cursor-pointer rounded px-5 py-1.5 shadow-md text-gray-600 font-semibold  whitespace-nowrap">
              <Plus size={16} className="inline-block" /> Add A New Company
              Migration
            </button>
          </Link>
         <Link className="cursor-pointer max-md:w-full" to={"/dashboard/payment"}> <button className="border w-full rounded px-5 py-1.5 shadow-md text-gray-600 font-semibold  whitespace-nowrap cursor-pointer">
        ...
        <table className="w-full mt-3 max-lg:hidden">
          ...
          <tbody>
            {orders?.length > 0
              ? orders?.map((order) => (
                  <tr key={order?._id}>
                    ...
                    <td className={`px-4 py-2 `}>
                     <span className={`border-none shadow-none rounded inline-block w-max py-0.5 px-2.5 text-sm  bg-gray-200  font-semibold  ${order?.status === "working" && "bg-orange-300 text-orange-900" } ${order?.status === "fulfilled" && "bg-green-300 text-green-900" } ${order?.status === "canceled" && "bg-red-400 text-red-900" }`}> {order?.status}</span>
                    </td>
                    <td className="text-nowrap p-3 md:py-3 md:px-5 text-start text-gray-600">
                      {" "}
              ...
              : <tr><td colSpan={6} className="text-center font-semibold py-3">No Data Found</td></tr>}
