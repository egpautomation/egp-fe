    ...
    <div>
      <div className="flex justify-between mt-5">
        ...
        <div className="flex items-center justify-center gap-2">
          <Link className=" inline-block" to={"/dashboard/bankService/account-holder-details"}>
          ...
          <table className="mt-5 w-full">
            <thead>
              <tr className="bg-primary text-primary-foreground">
                <th className="whitespace-nowrap px-4 py-2 text-start">Account Name</th>
                <th className="whitespace-nowrap px-4 py-2 text-start ">Account Holder Name</th>
                <th className="whitespace-nowrap px-4 py-2 text-start ">Account Number</th>
                <th className="whitespace-nowrap px-4 py-2 text-start rounded-tr">E-GP Email</th>
                ...
                data?.map((item, idx) => (
                  <tr key={idx} className={`border ${idx % 2 == 1 && "bg-gray-100"}`}>
                    ...
                    <td className={`px-4 py-2 flex items-center gap-2.5`}>
                      <Link to={`/dashboard/bankService/edit-account-details/${item?._id}`}>
                        <Edit />
                      </Link>{" "}
                      <DeleteDataModal
                        setReload={setReload}
