...
export default function ALLBOQ() {
  ...
  return (
    <table className="mt-5 w-full max-lg:hidden">
      <thead>
        <tr className="bg-primary text-primary-foreground">
          {[
            ...
            <th key={index} className="whitespace-nowrap px-2 py-2 text-start relative group">
              <span>{item.split("").length > 12 ? item.slice(0, 12) + "..." : item}</span>
              {/* Hover Popup */}
      ...
      <tbody>
        {data?.map((item, idx) => (
          <tr className={`border ${idx % 2 == 1 && "bg-gray-100"}`} key={idx}>
