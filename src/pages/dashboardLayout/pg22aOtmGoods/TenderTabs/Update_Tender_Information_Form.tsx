...
export const UpdateTenderInformationForm = ({ egpEmail, preSelectedContractId }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const dataToSubmit = {
      ...
      TentativeCompletionDate: tenderData?.TentativeCompletionDate || formData.TentativeCompletionDate,
    ...
    <div className="bg-gray-50 p-4 sm:p-6 lg:p-8">
      <form onSubmit={handleSubmit} className="max-w-7xl mx-auto bg-white p-6 sm:p-8 rounded-xl shadow-lg">
        <div className="mb-10 text-center">
          ...
          <p className="text-gray-500 mt-2">Select a contract from the list below to update its details.</p>
        </div>
            ...
            <PopoverContent className="w-[672px] p-0" align="start">
              <Command>
                ...
                <CommandList>
                  ...
                  <CommandGroup>
                    {Array.isArray(existingContracts) && existingContracts.map((contract, index) => (
                      <CommandItem
                        key={contract._id}
                      ...
                      >
                        ...
                        />
                        {index + 1}. Tender ID: {contract.tenderId} - FY: {contract.financialYear} ({contract.Status_Complite_ongoing})
                      </CommandItem>
                    ))}
                  </CommandGroup>
                </CommandList>
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <FormField id="development_partner" label="Development Partner">
                <Input name="development_partner" value={formData?.development_partner || "Not Available"} disabled />
              </FormField>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <FormField id="LtmLicenseNameCode" label="LTM License Name Code">
                <Input name="LtmLicenseNameCode" value={formData?.LtmLicenseNameCode || "NOt Available"} disabled />
              </FormField>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              ...
              <FormField id="PE_officialDesignation" label="PE Official Designation">
                <Input name="PE_officialDesignation" value={formData?.PE_officialDesignation || "Not Available"} disabled />
              </FormField>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
              <FormField id="ProjectName" label="Project Name">
                <Input name="ProjectName" value={formData?.ProjectName || "NOt Available"} disabled />
              </FormField>
              <FormField id="sourceOfFunds" label="Source of Funds">
                <Input name="sourceOfFunds" value={formData?.sourceOfFunds || "Not Available"} disabled />
              </FormField>
              ...
              <FormField id="selectedTenderCategory" label="Tender Category">
                <Input name="selectedTenderCategory" value={formData?.selectedTenderCategory || "Not Available"} disabled />
              </FormField>
              <FormField id="tender_subCategories" label="Tender Sub-Categories">
                <Input name="tender_subCategories" value={formData?.tender_subCategories || "Not Available"} disabled />
              </FormField>
              <FormField id="identificationOfLot" label="Identification of Lot">
                <Input name="identificationOfLot" value={formData?.identificationOfLot || "Not Available"} disabled />
              </FormField>
              <div className="sm:col-span-2">
                <FormField id="descriptionOfWorks" label="Description of Works">
                  <Textarea name="descriptionOfWorks" placeholder="Provide a detailed description of the works..." value={formData?.descriptionOfWorks} disabled />
                </FormField>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-8">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              <FormField id="publicationDateTime" label="Publication Date">
                <Input name="publicationDateTime" type="text" value={formData?.publicationDateTime || "Not Available"} disabled />
              </FormField>
              <FormField id="openingDateTime" label="Opening Date">
                <Input name="openingDateTime" type="text" value={formData?.openingDateTime || "Not Available"} disabled />
              </FormField>
              <FormField id="estimatedCost" label="Estimated Cost">
                <Input name="estimatedCost" type="number" value={formData?.estimatedCost} disabled />
              </FormField>
              ...
              <FormField id="ProjectCode" label="Project Code">
                <Input name="ProjectCode" value={formData?.ProjectCode || "Not Available"} disabled />
              </FormField>
              <FormField id="TentativeStartDate" label="Tentative Start Date">
                <Input name="TentativeStartDate" type="text" value={formData?.TentativeStartDate || "Not Available"} disabled />
              </FormField>
              <FormField id="TentativeCompletionDate" label="Tentative Completion Date">
                <Input name="TentativeCompletionDate" type="date" value={formData?.TentativeCompletionDate || "Not Available"} disabled />
              </FormField>
            </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 items-end">
              ...
              <div className="flex flex-col space-y-2">
                ...
                <Input id="revisedContractValue" name="revisedContractValue" type="number" value={formData.revisedContractValue} onChange={(e) => handleChange(e.target.name, e.target.value)} disabled={useInitialContractValue} />
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            ...
            <div className="flex flex-col space-y-2">
              ...
              <Input id="companyId_Tender" name="companyId_Tender" value={useUniqueIdForCompany ? egpListedCompany?.companyUniqueEGP_ID : formData.companyId_Tender} onChange={(e) => handleChange(e.target.name, e.target.value)} disabled={useUniqueIdForCompany} />
            </div>
        ...
        <div className="mb-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-12 gap-3 mb-4">
            <div className="md:col-span-5">
              ...
              <Combobox
                options={subCategories.map((cat, idx) => ({ value: cat.sub_cat_name, label: cat.sub_cat_name, key: `${cat.sub_cat_name}-${idx}` }))}
