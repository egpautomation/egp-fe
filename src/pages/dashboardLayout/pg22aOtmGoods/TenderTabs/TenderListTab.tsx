    ...
    return (
        <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-56">
                {options.map((option) => (
                    <DropdownMenuItem key={option} onSelect={(e) => e.preventDefault()} onClick={() => handleSelect(option)}>
...
export const TenderListTab = ({ data, loading }) => {
    const allFinancialYears = useMemo(() => [...new Set(data.map(item => item.financialYear))].sort((a, b) => b.localeCompare(a)), [data]);
    ...
    const groupedAndFilteredData = useMemo(() => {
        ...
        return Object.keys(grouped).sort((a, b) => b.localeCompare(a)).reduce(
            (obj, key) => {
                obj[key] = grouped[key];
                return obj;
            },
    ...
    const tableHeaders = ["S. No.", "Tender ID", "Package No", "Procuring Entity", "Description of Works", "JV Share (%)", "Payment Amount", "Status"];
            ...
            {loading ? (
                ...
                Object.entries(groupedAndFilteredData).map(([year, groupData]) => (
                    <div key={year} className="mb-8">
                        <h4 className="text-lg font-bold text-gray-700 bg-orange-100 p-2 rounded-t-md">Financial Year: {year}</h4>

                        {/* ডেস্কটপ ভিউ */}
                        <div className="overflow-x-auto hidden lg:block">
                            <table className="w-full text-sm text-left">
                                <thead className="bg-gray-700 text-white">
                                    <tr>{tableHeaders.map(h => <th key={h} className="px-4 py-3 font-medium whitespace-nowrap">{h}</th>)}</tr>
                                </thead>
                                <tbody className="bg-white divide-y">
                                    {groupData.tenders.map((tender, index) => (
                                        <tr key={tender.tenderId} className="hover:bg-gray-50">
                                            ...
                                            <td className="px-4 py-3 text-right font-mono">{parseFloat(tender.actualPaymentJvShare).toLocaleString('en-IN')}</td>
                                            <td className="px-4 py-3"><StatusBadge status={tender.Status_Complite_ongoing} /></td>
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot className="bg-gray-200 font-bold">
                                    <tr>
                                        <td colSpan={tableHeaders.length - 2} className="px-4 py-3 text-right">Total for {year}</td>
                                        <td className="px-4 py-3 text-right font-mono">{groupData.total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                        <td></td>
                                    </tr>
                                </tfoot>
                            </table>
                        ...
                        <div className="lg:hidden bg-white p-2 rounded-b-md">
                            {groupData.tenders.map((tender, index) => (
                                <div key={tender.tenderId} className="border rounded-lg p-3 space-y-2 mb-3 shadow-sm">
                                    <div className="flex justify-between border-b pb-2">
                                        <h5 className="font-bold">{tender.tenderId}</h5>
                                        <span className="text-sm">#{index + 1}</span>
                                    </div>
                                    <p><strong>Package No:</strong> {tender.packageNo}</p>
                                    <p><strong>Payment Amount:</strong> BDT {parseFloat(tender.actualPaymentJvShare).toLocaleString('en-IN')}</p>
                                    <p><strong>Status:</strong> <StatusBadge status={tender.Status_Complite_ongoing} /></p>
                                </div>
                            ))}
                            <div className="text-right font-bold p-3 bg-gray-200 rounded-md">
                                Total for {year}: BDT {groupData.total.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                            </div>
                        </div>
                    </div>
                ))
