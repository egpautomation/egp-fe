...
export const TurnoverHistoryTab = ({ yearlyTotals }) => {
    ...
    const sumOfBestYears = useMemo(() => bestYears.reduce((sum, year) => sum + year.amount, 0), [bestYears]);
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            Object.keys(saveData).forEach(key => {
                ...
                if (key.includes('Amount') || key === 'AverageAnnualTurnover' || key === 'tdsYearFinancial' || key === 'tdsYearBest') {
            ...
            cleanPayload.AverageAnnualTurnover = (isNaN(avgTurnoverNum) || !isFinite(avgTurnoverNum)) ? 0 : Number(parseFloat(avgTurnoverNum.toFixed(2)));
            ...
            for (let i = 1; i <= 5; i++) {
                ...
                cleanPayload[nameKey] = (nameValue === undefined || nameValue === null) ? '' : String(nameValue);

                // Ensure amount is a valid number
                const amount = parseFloat(String(amountValue || 0));
                cleanPayload[amountKey] = (isNaN(amount) || !isFinite(amount)) ? 0 : Number(parseFloat(amount.toFixed(2)));
            }

            // Verify payload does NOT contain jobOrder or other fields
            const allowedFields = [
                'tdsYearFinancial', 'tdsYearBest', 'AverageAnnualTurnover',
                'FinancialYear1Name', 'FinancialYear1Amount',
                'FinancialYear2Name', 'FinancialYear2Amount',
                'FinancialYear3Name', 'FinancialYear3Amount',
                'FinancialYear4Name', 'FinancialYear4Amount',
                'FinancialYear5Name', 'FinancialYear5Amount'
            ];
            ...
            const errorMessage = error?.response?.data?.message || error?.message || 'Failed to save turnover history';
    ...
    return (
        <div className="bg-purple-50 p-4 rounded-lg space-y-6">
            <h3 className="font-semibold text-purple-800 text-xl">Annual Turnover History & Calculation</h3>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-4 rounded-lg shadow-sm">
                <div>
                    ...
                    <Input id="tdsRequiredFY" type="number" value={tdsRequiredFY} onChange={(e) => setTdsRequiredFY(e.target.value)} />
                </div>
                <div>
                    <Label htmlFor="tdsRequiredBestYear">TDS Required (Best N Years)</Label>
                    <Input id="tdsRequiredBestYear" type="number" value={tdsRequiredBestYear} onChange={(e) => setTdsRequiredBestYear(e.target.value)} />
                </div>
            </div>

            {/* Tabella degli anni filtrati */}
            <div>
                <h4 className="font-semibold text-lg mb-2">Turnover for the Last {tdsRequiredFY} Financial Years</h4>
                            ...
                            {filteredAndSortedYears.length > 0 ? (
                                filteredAndSortedYears.map((item, index) => (
                                    <tr key={item.year} className="border-b">
                                        ...
                                        <td className="p-3 text-right font-mono">{item.amount.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                                    </tr>
                                ))
                            ) : (
                                <tr>
            ...
            <div>
                <h4 className="font-semibold text-lg mb-2">Best {tdsRequiredBestYear} Years Based on Turnover</h4>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm text-left bg-white rounded-lg shadow-sm">
                        <thead className="bg-gray-700 text-white">
                            <tr className="text-left">
                        ...
                        <tbody>
                            {bestYears.length > 0 ? (
                                ...
                                <tr>
                                    <td colSpan="3" className="text-center p-4 text-gray-500">No data to determine best years.</td>
                                </tr>
                            )}
                        </tbody>
                        <tfoot className="bg-gray-200 font-bold">
                            <tr>
                                <td colSpan="2" className="p-3 text-right">Sum of Best Years</td>
                                <td className="p-3 text-right font-mono">{sumOfBestYears.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</td>
                            </tr>
                        </tfoot>
                    </table>
                </div>
            ...
            <div className="bg-white p-4 rounded-lg shadow-md text-center space-y-4">
                <div>
                    ...
                    <p className="text-2xl font-bold text-green-600">
                        BDT {averageAnnualTurnover.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                    </p>
                </div>
                <Button
                    onClick={handleSave}
