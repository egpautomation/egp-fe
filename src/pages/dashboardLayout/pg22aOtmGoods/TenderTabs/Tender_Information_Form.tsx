...
export const TenderInformationForm = ({ egpEmail, setReload }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    const requiredFields = [
      ...
      {
        key: "workCompletationCertificateFileName",
    ...
    try {
      ...
      if (companyId) {
        ...
        if (data?.data && Array.isArray(data.data)) {
          const existingTender = data.data.find(
            (contract) => contract.tenderId === formData.tenderId
          );
          if (existingTender) {
            const status = existingTender.Status_Complite_ongoing;
            toast.error(
              `This Tender ID already exists as ${status}. Please use Update Tender Info to modify it.`
            );
            setLoading(false);
            return;
          }
    ...
    try {
      await createData(`${config.apiBaseUrl}/contract-information/create-contract-information`, [
        dataToSubmit,
      ]);
      ...
      setFeedback({
        message: `Error: ${error.message || "Failed to create tender"}`,
        type: "error",
      });
    } finally {
      setLoading(false);
      ...
      >
        <div className="mb-10 text-center">
          ...
          <p className="text-gray-500 mt-2">
            Please fill out all the necessary details accurately.
          </p>
        </div>
