...
const StatusBadge = ({ status }) => (
    <span className={`px-3 py-1 text-xs font-semibold rounded-full ${status === 'Completed'
...
export const OngoingTenderTab = ({ data, loading, setReload }) => {
    ...
    const ongoingData = useMemo(() => data.filter(item => item.Status_Complite_ongoing === 'ongoing'), [data]);
    ...
    const handleUpdateNYear = async (contractId, contractItem) => {
        ...
        try {
            ...
            const errorMessage = error?.response?.data?.message || error?.message || 'Failed to update N Year';
    ...
    const columns = [
        ...
        { Header: 'Revised Contract Value', accessor: item => parseFloat(item.revisedContractValue || 0).toLocaleString('en-IN') },
        { Header: 'Payment Amount', accessor: item => parseFloat(item.actualPaymentJvShare || 0).toLocaleString('en-IN') },
        { Header: 'JV Share (%)', accessor: 'jvShare' },
        { Header: 'N Year', accessor: 'nYear', isEditable: true },
        {
            Header: 'Remaining Works', accessor: item => {
                const value = parseFloat(item.RemainingWorks || 0);
                return value > 0 ? value.toLocaleString('en-IN') : '0';
            }
        },
        {
            Header: 'Works in Hand', accessor: item => {
                const value = parseFloat(item.WorksInHand || 0);
                return value > 0 ? value.toLocaleString('en-IN') : '0';
            }
        },
        {
            Header: 'Works in Hand / N Year', accessor: item => {
                ...
                return value > 0 ? value.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : '0.00';
            }
        },
    ...
    const tableData = useMemo(() => {
        return Object.entries(groupedData).map(([year, tenders]) => {
            const totals = tenders.reduce((acc, item) => {
    ...
    const grandTotal = useMemo(() => {
        return tableData.reduce((acc, group) => {
            ...
            {loading ? (
                ...
                tableData.map((group) => (
                    <div key={group.year} className="mb-6">
                        <h4 className="text-lg font-bold text-gray-700 bg-green-100 p-2 rounded-t-md">Financial Year: {group.year}</h4>
                        ...
                        <div className="overflow-x-auto hidden lg:block">
                            <table className="w-full text-sm text-left">
                                <thead className="bg-gray-700 text-white">
                                    <tr>
                                        {columns.map((col) => {
                                            ...
                                            return <th key={col.Header} className={`px-4 py-3 font-medium whitespace-nowrap ${widthClass}`}>{col.Header}</th>;
                                        })}
                                    </tr>
                                </thead>
                                    ...
                                    {group.tenders.map((item) => (
                                        <tr key={item.tenderId || item._id} className="hover:bg-gray-50">
                                            {columns.map((column, index) => {
                                                ...
                                                const cellValue = column.Cell ? column.Cell({ row: { original: item } }) : (typeof column.accessor === 'function' ? column.accessor(item) : item[column.accessor]);
                                                // Apply different styling based on column type
                                                const isNumericColumn = ['Revised Contract Value', 'Payment Amount', 'JV Share (%)', 'Remaining Works', 'Works in Hand', 'Works in Hand / N Year'].includes(column.Header);
                                                ...
                                                return <td key={index} className={cellClass} title={isDescriptionColumn ? displayValue : undefined}>{displayValue}</td>;
                                            })}
                                        </tr>
                                    ))}
                                </tbody>
                                <tfoot className="bg-gray-200 font-bold">
                                    <tr>
                                        <td colSpan={6} className="px-4 py-3 text-right">Total for {group.year}</td>
                                        <td className="px-4 py-3 text-right font-mono">{group.totals.revisedContractValue.toLocaleString('en-IN')}</td>
                                        <td className="px-4 py-3 text-right font-mono">{group.totals.paymentAmount.toLocaleString('en-IN')}</td>
                                        <td className="px-4 py-3 text-right"></td>
                                        <td className="px-4 py-3 text-right"></td>
                                        <td className="px-4 py-3 text-right font-mono">{group.totals.totalRemainingWorks.toLocaleString('en-IN')}</td>
                                        <td className="px-4 py-3 text-right font-mono">{group.totals.totalWorksInHand.toLocaleString('en-IN')}</td>
                                        <td className="px-4 py-3 text-right font-mono">
                                            {group.totals.totalWorksInHandPerNYear.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                        </td>
                                        <td className="px-4 py-3 text-right"></td>
                        ...
                        <div className="lg:hidden bg-white p-2 rounded-b-md">
                            {group.tenders.map((item, index) => (
                                <div key={item.tenderId || item._id} className="border rounded-lg p-3 space-y-2 mb-3 shadow-sm">
                                    <div className="flex justify-between border-b pb-2">
                                        <h5 className="font-bold">{item.tenderId}</h5>
                                        <span className="text-sm">#{index + 1}</span>
                                    </div>
                                    <p><strong>Package No:</strong> {item.packageNo}</p>
                                    <p><strong>Description:</strong> {item.descriptionOfWorks}</p>
                                    <p><strong>Payment Amount:</strong> BDT {parseFloat(item.actualPaymentJvShare || 0).toLocaleString('en-IN')}</p>
                                    ...
                                    <p><strong>Remaining Works:</strong> BDT {parseFloat(item.RemainingWorks || 0) > 0 ? parseFloat(item.RemainingWorks || 0).toLocaleString('en-IN') : '0'}</p>
                                    <p><strong>Works in Hand:</strong> BDT {parseFloat(item.WorksInHand || 0) > 0 ? parseFloat(item.WorksInHand || 0).toLocaleString('en-IN') : '0'}</p>
                                    <p><strong>Works in Hand / N Year:</strong> BDT {
                                        (() => {
                                            const worksInHand = parseFloat(item.WorksInHand || 0);
                                            const nYearValue = parseFloat(nYear[item._id] || item.nYear || 0);
                                            const value = nYearValue > 0 ? worksInHand / nYearValue : 0;
                            ...
                            <div className="text-right font-bold p-3 bg-gray-200 rounded-md space-y-1">
                                <div>Total for {group.year}: BDT {group.totals.paymentAmount.toLocaleString('en-IN')}</div>
                                <div>Total Works in Hand: BDT {group.totals.totalWorksInHand.toLocaleString('en-IN')}</div>
                                <div>Total Works in Hand / N Year = {group.totals.totalWorksInHandPerNYear.toLocaleString('en-IN')}</div>
                            </div>
                        </div>
                    </div>
                ))
                ...
                <div className="mt-6">
                    ...
                    <div className="hidden lg:block overflow-x-auto">
                        <table className="w-full text-sm text-left">
                            <tfoot className="bg-gray-700 text-white font-bold text-base">
                                <tr>
                                    <td colSpan={6} className="px-4 py-4 text-right">GRAND TOTAL (All Years)</td>
                                    ...
                                    <td className="px-4 py-4 text-right font-mono text-yellow-300">
                                        {grandTotal.totalWorksInHandPerNYear.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}
                                    </td>
                                    <td className="px-4 py-4 text-right"></td>
                                </tr>
                            </tfoot>
                    ...
                    <div className="lg:hidden bg-green-700 text-white p-4 rounded-lg space-y-2 font-bold">
                        ...
                        <div className="flex justify-between text-yellow-300 text-lg pt-2">
                            ...
                            <span>{grandTotal.totalWorksInHandPerNYear.toLocaleString('en-IN', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
                        </div>
                    </div>
                </div>
            )}
