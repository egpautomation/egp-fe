...
const mockTendererFormData = {
  ...
  part3: [{ financialYear: "2022-2023", amount: 180000000 }],
  part4: [
  ...
  part5: {
    completedWorks: [
      {
        slNo: 1,
        nameOfWork: "Dhaka-Mymensingh Highway Upgrade",
        valueOfWork: 120000000,
        completionDate: "2023-05-20",
        certificateFile: "completion_cert_1.pdf",
      },
    ],
    ongoingWorks: [
      {
        slNo: 1,
        nameOfWork: "Matarbari Power Plant Civil Works",
        valueOfWork: 800000000,
        awardDate: "2023-02-10",
        peName: "CPGCBL",
        agreementFile: "agreement_matarbari.pdf",
      },
    ],
  },
  activities: [
...
export const TendererFormPreview = ({
  ...
  const filteredContractsB = useMemo(() => {
    ...
    return completedContracts.filter((contract) => {
      ...
      const rawValue = contract?.contractValue || contract?.revisedContractValue;
      const value =
        typeof rawValue === "number" ? rawValue : Number(String(rawValue || "").replace(/,/g, ""));
  ...
  const handleSearch = (query) => {
    ...
    const results = ids.map((id) => {
      const contract = historySource.find(
        (c) => String(c?.tenderId || "").trim() === id || String(c?.contractNo || "").trim() === id
      );
      ...
      if (!contract) {
        return {
          id,
          found: false,
          reason: "Tender ID not found in your completed contracts history.",
        };
      }
  ...
  return (
    <div className="bg-gray-50 p-4 space-y-6">
        ...
        {similarNatureMainCategory || similarNatureSubCategory ? (
          <div className="text-xs text-gray-600 mb-2">
            ...
            {similarNatureMainCategory ? (
              <>
                <span className="font-semibold text-gray-800">{similarNatureMainCategory}</span>{" "}
                (Main Category)
              </>
            ) : (
              <>
                <span className="font-semibold text-gray-800">{similarNatureSubCategory}</span> (Sub
                Category Match)
              </>
            )}
            {similarNatureMainCategory && similarNatureSubCategory && (
              ...
              {similarNatureMainCategory || similarNatureSubCategory ? (
                filteredSimilarContracts.length > 0 ? (
                  filteredSimilarContracts.map((c, index) => (
                    <tr
                      key={c?._id || `${c?.tenderId || "contract"}-${index}`}
                      className={`border-b ${confirmedContract?._id === c?._id ? "bg-blue-50" : ""}`}
                    >
                      ...
                      <td className="p-2">
                        {formatDateSafe(
                          pickFirstTruthy(
                            c?.contractPeriodExtendedUpTo,
                            c?.contractEndDate,
                            c?.intendedCompletionDate
                          )
                        )}
                      </td>
                      <td className="p-2 font-mono text-right">
                        {formatMoney(pickFirstTruthy(c?.contractValue, c?.revisedContractValue))}
                      </td>
      ...
      <Section title="Part 2: Experience in Similar Contracts B">
        <div className="text-xs text-gray-600 mb-2">
          Filtering by: <span className="font-semibold text-gray-800">All Categories</span> (Value ≥
          25 Lakh BDT)
        </div>
        ...
        <div className="mb-4 bg-white p-3 border rounded shadow-sm relative">
          <label className="block text-xs font-semibold text-gray-700 mb-1">
            Check Eligibility by Tender ID
          </label>
          <input
            type="text"
            value={searchQuery}
              ...
              {suggestions.map((s, idx) => (
                ...
                >
                  ...
                  <span className="text-gray-500 ml-2">
                    {" "}
                    -{" "}
                    {s.descriptionOfWorks
                      ? s.descriptionOfWorks.substring(0, 40) + "..."
                      : "No Desc"}
                  </span>
                </div>
              ))}
              ...
              {searchResults.map((res, idx) => (
                ...
                >
                  <div className="flex items-center mb-2">
                    <div
                      className={`mr-2 font-bold ${res.isValid ? "text-green-600" : "text-red-500"}`}
                    >
                      {res.isValid ? "✓" : "✕"}
                    </div>
                  ...
                  {!res.found ? (
                    <div className="text-gray-600 ml-6">
                      Not found in your completed contracts history.
                    </div>
                  ) : (
                        ...
                        <tbody className="text-xs text-gray-700">
                          <tr>
                            ...
                            <td className="p-2 border-t">
                              {res.contract?.descriptionOfWorks
                                ? res.contract.descriptionOfWorks.substring(0, 50) + "..."
                                : res.contract?.ProjectName
                                  ? res.contract.ProjectName.substring(0, 50) + "..."
                                  : "N/A"}
                            </td>
                            <td className="p-2 border-t">
                              {roleLabel(res.contract?.Role_in_Contract)}
                            </td>
                            <td className="p-2 border-t">
                              {formatDateSafe(
                                pickFirstTruthy(
                                  res.contract?.noaIssueDate,
                                  res.contract?.contractSigningDate,
                                  res.contract?.commencementDate
                                )
                              )}
                            </td>
                            <td className="p-2 border-t">
                              {formatDateSafe(
                                pickFirstTruthy(
                                  res.contract?.contractPeriodExtendedUpTo,
                                  res.contract?.contractEndDate,
                                  res.contract?.intendedCompletionDate
                                )
                              )}
                            </td>
                            <td className="p-2 border-t font-mono">
                              {formatMoney(
                                pickFirstTruthy(
                                  res.contract?.contractValue,
                                  res.contract?.revisedContractValue
                                )
                              )}
                            </td>
                            <td className="p-2 border-t">
                              {res.contract?.procuringEntityName || "N/A"}
                            </td>
                            <td className="p-2 border-t text-center align-middle">
                              {res.isValid ? (
                                <div className="flex flex-col items-center justify-center gap-1">
                                  <span className="text-green-600 font-medium text-[10px] leading-tight">
                                    ✓ Eligible
                                  </span>
                                  <Button
                                    size="sm"
                                    variant={
                                      confirmedContract?._id === res.contract?._id
                                        ? "default"
                                        : "outline"
                                    }
          ...
          {totalPagesB > 1 && (
            <div className="flex justify-between items-center mt-4 px-2">
              <div className="text-xs text-gray-500">
                Showing {(currentPageB - 1) * itemsPerPageB + 1} to{" "}
                {Math.min(currentPageB * itemsPerPageB, filteredContractsB.length)} of{" "}
                {filteredContractsB.length} entries
              </div>
      ...
      <Section title="Calculation of Specific Experience (Part 2)">
        <table className="w-full text-sm text-left">
          ...
          <tbody>
            ...
            {(() => {
              // 1. Required Category
              const requiredSubCategory = extractSubCategoryFromTypesOfSimilarNature(
                liveTender?.typesOfSimilarNature
              );
              ...
              const requiredMainCategory =
                subToMain.get(normalizeKey(requiredSubCategory)) ||
                similarNatureMainCategory ||
                "N/A";
              ...
              const similarNatureValueStr = liveTender?.typesOfSimilarNature?.includes(" : ")
                ...
                : "2500000";
              const requiredValue =
                Number(String(similarNatureValueStr).replace(/,/g, "")) || 2500000;
              ...
              if (confirmedContract) {
                const contractCats = Array.isArray(confirmedContract?.tenderCategories)
                  ? confirmedContract.tenderCategories
                  : [];
              ...
              // Value Match?
              const submittedRawValue = confirmedContract
                ? confirmedContract?.contractValue || confirmedContract?.revisedContractValue
                : 0;
              const submittedValue =
                typeof submittedRawValue === "number"
                  ? submittedRawValue
                  : Number(String(submittedRawValue || "").replace(/,/g, ""));
              const valuePass = submittedValue >= requiredValue;

              // Time Match?
              const completionDateStr = confirmedContract
                ? pickFirstTruthy(
                    confirmedContract?.contractPeriodExtendedUpTo,
                    confirmedContract?.contractEndDate,
                    confirmedContract?.intendedCompletionDate
                  )
                : null;
                ...
                <>
                  ...
                  <tr className="border-b">
                    <td className="p-2 bg-gray-50 font-medium">
                      Types of Similar Nature (Main Category)
                    </td>
                    <td className="p-2 text-right">{requiredMainCategory}</td>
                    <td className="p-2 text-right text-gray-600">
                      {confirmedContract ? (
                        categoryMatch ? (
                          <span className="text-green-600 font-medium">Matches Requirement</span>
                        ) : (
                          <span className="text-red-600">Categories Do Not Match</span>
                        )
                      ) : (
                        "Select a contract"
                      )}
                  ...
                  {/* Row 3: Minimum Year */}
                  <tr
                    className={`border-b ${confirmedContract ? (timePass ? "bg-green-50" : "bg-red-50") : ""}`}
                  >
                    <td className="p-2 font-medium">Minimum Year of Similar Work</td>
                    <td className="p-2 text-right">
                      {minY} Years ({requiredDays} Days)
                    </td>
                    <td className="p-2 text-right">
                      {confirmedContract
                        ? daysAgo !== null
                  ...
                  <tr
                    className={`border-t-2 ${confirmedContract ? (overallPass ? "bg-green-100" : "bg-red-100") : "bg-gray-50"}`}
                  >
                    <td className="p-2 font-bold text-gray-900 uppercase" colSpan={3}>
                      Overall Result
                    </td>
                    <td
                      className={`p-2 font-bold text-right text-lg ${confirmedContract ? (overallPass ? "text-green-700" : "text-red-600") : "text-gray-400"}`}
                    >
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 font-semibold">
                  Calculation of Minimum Average Annual Construction Turnover
                </th>
                <th className="p-2 font-semibold text-right">Amount (BDT)</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr className="border-b bg-blue-50">
                <td className="p-2 font-medium text-blue-900">
                  Required Average Annual Construction Turnover
                </td>
              ...
              {(() => {
                ...
                return (
                  <tr className={`border-b ${isPass ? "bg-green-100" : "bg-red-50"}`}>
                    ...
                    <td
                      className={`p-2 font-bold text-right ${isPass ? "text-green-700" : "text-red-600"}`}
                    >
                      {isPass ? "Pass" : "Fail"}
                    </td>
                  </tr>
        ...
        <Section title="Part 4: Calculation of Tender Capacity">
          <table className="w-full text-sm text-left">
            <thead className="bg-gray-100">
              <tr>
                <th className="p-2 font-semibold">Calculation of Tender Capacity</th>
                <th className="p-2 font-semibold text-right">Amount Available (BDT)</th>
              </tr>
            </thead>
            <tbody>
              ...
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.estimatedCost
                    ? `BDT ${Number(financialInfo.estimatedCost).toLocaleString("en-IN", { minimumFractionDigits: 0 })}`
                    : "N/A"}
                </td>
              </tr>
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.documentPrice
                    ? `BDT ${Number(financialInfo.documentPrice).toLocaleString("en-IN", { minimumFractionDigits: 0 })}`
                    : "N/A"}
                </td>
              </tr>
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.tenderSecurity
                    ? `BDT ${Number(financialInfo.tenderSecurity).toLocaleString("en-IN", { minimumFractionDigits: 0 })}`
                    : "N/A"}
                </td>
              </tr>
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.turnoverAmount
                    ? `BDT ${Number(financialInfo.turnoverAmount).toLocaleString("en-IN", { minimumFractionDigits: 0 })}`
                    : "N/A"}
                </td>
              </tr>
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.liquidAssets
                    ? `BDT ${Number(financialInfo.liquidAssets).toLocaleString("en-IN", { minimumFractionDigits: 0 })}`
                    : "N/A"}
                </td>
              </tr>
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.tenderCapacity
                    ? `BDT ${Number(financialInfo.tenderCapacity).toLocaleString("en-IN", { minimumFractionDigits: 0 })}`
                    : "N/A"}
                </td>
              </tr>
              <tr className="border-b">
                ...
                <td className="p-2 text-right">
                  {financialInfo?.yearofsimilarexperience || "N/A"}
                </td>
              </tr>
              {financialInfo?.typesOfSimilarNature &&
              financialInfo.typesOfSimilarNature.includes(" : ") ? (
                <>
                  <tr className="border-b">
                    <td className="p-2">Types of Similar Nature</td>
                    <td className="p-2 text-right">
                      {financialInfo.typesOfSimilarNature.split(" : ")[0]}
                    </td>
                  </tr>
                  <tr className="border-b">
                    <td className="p-2">Type of Similar Nature Value</td>
                    <td className="p-2 text-right">
                      {financialInfo.typesOfSimilarNature.split(" : ")[1]}
                    </td>
                  </tr>
                </>
              ) : (
      ...
      <Section title="Part 5: Experience and Commitments">
        <div>
          <h4 className="font-semibold text-md mb-2 text-gray-600">
            (A) Completed Works (Last 5 years)
          </h4>
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead className="bg-gray-100">
                <tr>
                  {["SL", "Name of Works", "Value", "Completion Date", "Certificate"].map((h) => (
                    <th key={h} className="p-2 font-semibold">
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {data.part5.completedWorks.map((item) => (
                  <tr key={item.slNo} className="border-b">
                    <td className="p-2">{item.slNo}</td>
                    <td className="p-2">{item.nameOfWork}</td>
                    <td className="p-2 font-mono text-right">
                      {item.valueOfWork.toLocaleString("en-IN")}
                    </td>
                    <td className="p-2">{item.completionDate}</td>
        ...
        <div className="pt-4">
          <h4 className="font-semibold text-md mb-2 text-gray-600">
            (B) On-Going Works & Current Commitments
          </h4>
          <div className="overflow-x-auto">
            <table className="w-full text-xs text-left">
              <thead className="bg-gray-100">
                <tr>
                  {["SL", "Name of Works", "Value", "Award Date", "PE Name", "Agreement"].map(
                    (h) => (
                      <th key={h} className="p-2 font-semibold">
                        {h}
                      </th>
              ...
              <tbody>
                {data.part5.ongoingWorks.map((item) => (
                  <tr key={item.slNo} className="border-b">
                    <td className="p-2">{item.slNo}</td>
                    <td className="p-2">{item.nameOfWork}</td>
                    <td className="p-2 font-mono text-right">
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-gray-100">
            <tr>
              ...
              <th className="p-2 font-semibold text-center">Start (Days from Possession)</th>
              <th className="p-2 font-semibold text-center">Duration (Days)</th>
            </tr>
          </thead>
          <tbody>
            {data.activities.map((item, index) => (
              <tr key={index} className="border-b">
                ...
                <td className="p-2 text-center">{item.startDays}</td>
                <td className="p-2 text-center">{item.durationDays}</td>
              </tr>
            ))}
          </tbody>
        </table>
