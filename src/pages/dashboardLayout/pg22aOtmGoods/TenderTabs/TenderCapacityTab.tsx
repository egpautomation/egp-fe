...
export const TenderCapacityTab = ({
  ...
  const tdsRequiredFY =
    externalTdsRequiredFY !== undefined ? externalTdsRequiredFY : localTdsRequiredFY;
  const setTdsRequiredFY = externalSetTdsRequiredFY || setLocalTdsRequiredFY;
  const proposedProjectYear =
    externalProposedProjectYear !== undefined
      ? externalProposedProjectYear
      : localProposedProjectYear;
    ...
    const fetchExistingData = async () => {
      try {
        ...
        const searchResponse = await axiosInstance.get(
          `/tender-preparation?searchTerm=${encodeURIComponent(egpEmail)}&page=1&limit=10`
        );
        ...
        if (tenderPreparations.length > 0) {
          ...
          const tenderPrep =
            tenderPreparations.find((tp) => tp.egpEmail === egpEmail) || tenderPreparations[0];
  ...
  const financialYearOfMaxValue = useMemo(() => {
    ...
    const maxYear = relevantYears.reduce(
  ...
  const assessedCapacity = useMemo(() => {
    ...
    if (
      !Number(proposedProjectYear) &&
      !Number(tdsRequiredFY) &&
      existingData?.FinalAssessedTenderCapacity
    ) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      if (tenderPreparations.length === 0) {
        throw new Error(
          "Tender preparation not found for this email. Please create a tender preparation first."
        );
      }
      ...
      const cleanPayload = {
        ...
        Maximumvalue:
          isNaN(maxValue) || !isFinite(maxValue) ? 0 : Number(parseFloat(maxValue.toFixed(2))),
        valueB: isNaN(bValue) || !isFinite(bValue) ? 0 : Number(parseFloat(bValue.toFixed(2))),
        FinalAssessedTenderCapacity:
          isNaN(assessedCap) || !isFinite(assessedCap)
            ? 0
            : Number(parseFloat(assessedCap.toFixed(2))),
      ...
      if (response.data?.success) {
        ...
        if (updatedPreparations.length > 0) {
          const updatedPrep =
            updatedPreparations.find((tp) => tp.egpEmail === egpEmail) || updatedPreparations[0];
      ...
      const errorMessage =
        error?.response?.data?.message || error?.message || "Failed to update Tender Capacity";
  ...
  const calculationRows = [
    {
      label: `(A) Maximum value of Works performed in any one year during last ${tdsRequiredFY} years`,
      value: valueA,
    },
  ...
  return (
    <div className="bg-indigo-50 p-4 rounded-lg space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6 bg-white p-4 rounded-lg shadow-sm">
        <div>
          ...
          <Input
            ...
            type="number"
            value={tdsRequiredFY}
            onChange={(e) => setTdsRequiredFY(Number(e.target.value) || 0)}
          />
        </div>
        <div>
          <Label htmlFor="proposedProjectYear">Proposed Project Year (N)</Label>
          <Input
            id="proposedProjectYear"
            type="number"
            value={proposedProjectYear}
            onChange={(e) => setProposedProjectYear(Number(e.target.value) || 0)}
          />
        </div>
      </div>
          ...
          <tfoot className="bg-gray-200 font-bold text-lg">
            <tr>
              <td className="p-3 text-right">
                Assessed Tender Capacity = (A × N × {factor.toFixed(2)} - B)
              </td>
              <td className="p-3 text-right font-mono">
                {assessedCapacity.toLocaleString("en-IN", {
                  minimumFractionDigits: 2,
                  maximumFractionDigits: 2,
                })}
              </td>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md text-center">
        ...
        <p
          className={`text-3xl font-bold ${assessedCapacity >= 0 ? "text-green-600" : "text-red-600"}`}
        >
          BDT{" "}
          {assessedCapacity.toLocaleString("en-IN", {
            minimumFractionDigits: 2,
        ...
        {financialYearOfMaxValue && (
          <p className="text-sm text-gray-600 mt-2">
            Financial Year of Maximum Value:{" "}
            <span className="font-semibold">{financialYearOfMaxValue}</span>
          </p>
        )}
      </div>
      ...
      <div className="flex justify-center">
        <Button onClick={handleUpdate} disabled={isUpdating} className="px-8 py-2">
