...
export default function TenderBidAnalysisView() {
  ...
  const handleDownloadPdf = () => {
    ...
    // First page top margin adjustment
    doc.setFontSize(18);
    doc.setFont("helvetica", "bold");
    doc.setTextColor(33, 41, 54);
    doc.text("Tender STL Calculation Report", pageWidth / 2, 28, { align: "center" });

    doc.setFontSize(9);
    doc.setFont("helvetica", "normal");
    doc.setTextColor(100, 100, 100);
    doc.text(`Tender ID: #${record.tenderId} | Generated: ${new Date().toLocaleString()}`, pageWidth / 2, 35, { align: "center" });
    ...
    autoTable(doc, {
      ...
      margin: { left: 14, right: 14, top: 25, bottom: 25 },
    });
    ...
    if (biddersY > 240) {
      doc.addPage();
      biddersY = 25; // Adjusted down for header space
    }
    ...
    if (winnerY > 250) {
      doc.addPage();
      winnerY = 25; // Adjusted down for header space
    }
    ...
    // Apply header & footer branding to all pages
    const pageCount = doc.internal.getNumberOfPages();
    for (let i = 1; i <= pageCount; i++) {
      doc.setPage(i);

      // Top colored branding bar
      ...
      doc.setFontSize(8);
      doc.setFont("helvetica", "bold");
      doc.setTextColor(30, 87, 179);
      doc.text("eTenderBD.com", 14, 12);

      doc.setFont("helvetica", "normal");
      doc.setTextColor(120, 120, 120);
      doc.text("Smart Tender Analysis & Automation", pageWidth - 14, 12, { align: "right" });
      ...
      doc.text("This report is generated by eTenderBD.com - All Rights Reserved.", 14, pageHeight - 10);
