      ...
      >
        ...
        {parsed && (
          <>
            ...
            <button onClick={() => setPreview(!preview)} className="flex items-center gap-1 text-sm text-indigo-600 hover:underline">
              Preview {preview ? <ChevronUp size={12} /> : <ChevronDown size={12} />}
            </button>
            <Button onClick={handleImport} disabled={loading} className="bg-indigo-600 hover:bg-indigo-700 text-white text-sm px-5 py-2 rounded-lg shadow-sm">
              {loading ? <RefreshCw size={14} className="animate-spin mr-1 inline" /> : <Upload size={14} className="mr-1 inline" />}
              Import
            </Button>
            <button onClick={() => { setParsed(null); setFilename(""); }} className="text-gray-400 hover:text-red-500 transition">
              <X size={16} />
            </button>
          </>
        )}
        ...
        <div className="mt-4 overflow-x-auto max-h-52 rounded-lg border border-indigo-200 bg-white">
          <table className="text-sm w-full">
            <thead className="bg-indigo-100 text-indigo-700 sticky top-0">
              <tr>
                {Object.keys(parsed[0]).map((k) => <th key={k} className="px-3 py-1.5 text-left whitespace-nowrap">{k}</th>)}
              </tr>
            </thead>
            <tbody>
              {parsed.slice(0, 10).map((row, i) => (
                <tr key={i} className={i % 2 === 1 ? "bg-gray-50" : ""}>
                  {Object.values(row).map((v, j) => <td key={j} className="px-3 py-1.5 whitespace-nowrap text-gray-700">{String(v ?? "")}</td>)}
                </tr>
              ))}
            </tbody>
          </table>
          {parsed.length > 10 && <p className="text-center text-sm text-gray-400 py-1">+ {parsed.length - 10} more records</p>}
        </div>
      )}
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 mb-5">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="relative">
            ...
            />
            {searchTerm && (
              <button className="absolute right-2 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-700" onClick={() => setSearchTerm("")}>
                <X size={13} />
              </button>
            )}
          </div>
          ...
          <Button onClick={() => setShowImport(!showImport)} variant={showImport ? "default" : "outline"} className="h-10 text-base gap-1.5 px-4 rounded-xl shadow-sm">
            <FileJson size={14} /> JSON Import
          </Button>

          <Button onClick={downloadTenderIds} variant="outline" className="h-10 text-base gap-1.5 bg-emerald-50 border-emerald-200 text-emerald-700 hover:bg-emerald-100 px-4 rounded-xl shadow-sm">
          ...
          <tbody>
            {loading ? (
              ...
              tenders.map((item, idx) => (
                <tr key={item?._id || idx} className={`border-b border-gray-100 hover:bg-blue-50/40 ${idx % 2 === 1 ? "bg-gray-50/60" : ""}`}>
                  <td className="px-4 py-4 font-mono font-bold text-indigo-700">{item?.tenderId}</td>
                  ...
                  <td className="px-4 py-4 text-center text-sm">{formatDate(item?.publicationDateTime, "dd-MMM-yyyy")}</td>
                  <td className="px-4 py-4 text-center text-sm font-semibold text-red-600">{formatDate(item?.openingDateTime, "dd-MMM-yyyy")}</td>
                  <td className="px-4 py-3 text-center">
                    <Button onClick={() => handleDelete(item?._id)} variant="ghost" className="h-8 w-8 p-0 text-red-500 hover:text-red-700 hover:bg-red-50">
                      <X size={16} />
                    </Button>
                  </td>
                </tr>
              ...
              <tr>
                <td colSpan={7} className="text-center py-16 text-gray-400">No data found</td>
              </tr>
            )}
          </tbody>
        </table>
