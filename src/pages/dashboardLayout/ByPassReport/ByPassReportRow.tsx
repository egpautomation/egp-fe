...
export default function ByPassReportRow({ item, idx, data }) {
  const { data: egpListendCompany, loading } = useSingleData(
    `${config.apiBaseUrl}/egp-listed-company/get-by-mail?mail=${item?.egpMail}`
  );
  console.log(egpListendCompany);
  const bankName = egpListendCompany?.bankName || "NILL";

  const { data: tenderData } = useSingleData(
    `${config.apiBaseUrl}/tenders/tenderId/${item?.tenderIdNum}`
  );
  ...
  return (
    <tr key={idx} className={`border ${idx % 2 == 1 && "bg-gray-100"}`}>
      ...
      <td className="px-4 py-2">
        {item?.bankName ? item?.bankName : loading ? "loading..." : bankName}
      </td>
      <td className="px-4 py-2">
        {item?.liquidAssetsTenderAmount ? item?.liquidAssetsTenderAmount : liquidAssets}
      </td>
      <td className="px-4 py-2">{item?.activityDate1 || 3}</td>
      <td className="px-4 py-2">
        {item?.activityDate2 ||
          getActivityDays(item?.TentativeStartDate, item?.TentativeCompletionDate)}
      </td>
